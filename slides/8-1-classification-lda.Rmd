...
data(flea)
ggplot(data=flea, aes(x=tars1, y=aede1, color=species)) + 
  geom_point() +
  theme(aspect.ratio=1, legend.position="none") + xlab("") + ylab("")
ggplot(data=flea, aes(x=tars1, y=aede1)) + geom_point() + 
...
```{r olives, cache=FALSE, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.height=4, fig.width=4}
data(olive)
olive$region <- factor(olive$region, labels=c("South", "Sardinia", "North"))
ggplot(data=olive, aes(x=eicosenoic, y=linoleic, color=region, shape=region)) + 
...
ggplot(data=flea, aes(x=tars1, y=aede1, color=species, shape=species)) + 
  geom_point() + 
  theme(aspect.ratio=1) 
```
...
ggplot(data=olive.p, aes(x=eicosenoic, y=linoleic, color=region)) +
...
fl.m <- summarise(group_by(flea, species), tars1=mean(tars1),
                  aede1=mean(aede1))
ggplot(data=flea.p, aes(x=tars1, y=aede1, color=species)) + geom_point(alpha=I(0.2)) +
...
cr.m <- summarise(group_by(crab, sex), FL=mean(FL),
                  RW=mean(RW))
ggplot(data=crab.p, aes(x=FL, y=RW, color=sex)) + geom_point(alpha=I(0.2)) +
...
ggplot(data=olive.sub, aes(x=linoleic, fill=region, color=region)) +
...
#crab$FL_RW <- 3.01*crab$FL -3.86*crab$RW
crab$FL_RW <- crab$FL/crab$RW
ggplot(data=crab, aes(x=FL_RW, fill=sex, color=sex)) +
      geom_density(alpha=I(0.5)) +
  theme_bw() + theme(legend.position="bottom", aspect.ratio=1)
```
...
crab.lda <- lda(sex~FL_RW, data=crab, 
                 prior=c(0.5, 0.5))
crab.lda$means
```
...
ggplot(data=df, aes(x=x, y=y, colour=cl)) + geom_line() +
...
ggplot(data=crab, aes(x=d1, fill=sex)) + geom_histogram() + facet_wrap(~sex, ncol=1) 
```
...
ggplot(data=crab, aes(x=FL, y=RW, color=sex, shape=sex)) + geom_point() + 
  ylim(c(2,22)) + xlim(c(2,22)) + 
  geom_abline(intercept=30, slope=-1.26, linetype=2) + 
  geom_abline(intercept=0.753, slope=0.795) + 
  theme(aspect.ratio=1) 
```
...
$$
\bar{x}_1^TS_p^{-1}x_0 + \bar{x}_1^TS_p^{-1}\bar{x}_1/2 -(\bar{x}_2^TS_p^{-1}x_0 - \bar{x}_2^TS_p^{-1}\bar{x}_2/2 \geq ln(p_2)-ln(p_1)
$$

$$
\bar{x}_1^TS_p^{-1}x_0 + \bar{x}_1^TS_p^{-1}\bar{x}_1 + ln(p_1) \geq \bar{x}_2^TS_p^{-1}x_0 - \bar{x}_2^TS_p^{-1}\bar{x}_2 + ln(p_2)
$$
...
$$
\bar{x}_j^TS_p^{-1}x_0 + \bar{x}_j^TS_p^{-1}\bar{x}_j + ln(p_j)
$$
...
ggplot(data=o.p, aes(x=LD1, y=LD2, color=region)) + geom_point() + theme(aspect.ratio=1)
```
...
ggplot(data=f.p, aes(x=LD1, y=LD2, color=species)) + geom_point() + theme(aspect.ratio=1)
```
...
ggplot(data=c.p, aes(x=LD1, y=LD2, color=sp.sex)) + geom_point() + theme(aspect.ratio=1)
```
