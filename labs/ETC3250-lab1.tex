...
\usepackage{kpfonts,titling,hyperref,url,microtype}
...
\begin{verbatim}
boxplot(OJ[,c("SalePriceCH","SalePriceMM")])
plot(PriceDiff ~ Purchase, data=OJ)
plot(SalePriceCH ~ PriceCH, data=OJ)
plot(jitter(SalePriceCH) ~ jitter(PriceCH), data=OJ)
abline(0,1)
plot(jitter(SalePriceCH) ~ jitter(WeekofPurchase), data=OJ)
\end{verbatim}
...
\item There are often several ways of doing the same thing:
\begin{verbatim}
plot(PriceDiff ~ Purchase, data=OJ)
plot(OJ$PriceDiff ~ OJ$Purchase)
plot(OJ[,"PriceDiff"] ~ OJ[,"Purchase"])
plot(OJ[,"Purchase"], OJ[,"PriceDiff"])
\end{verbatim}
Are there any differences between these plots?
...
\begin{verbatim}
barplot(table(pisa$ST03Q01), names=month.abb)
\end{verbatim}
...
barplot(table(pisa$ST03Q01)/monthdays, names=month.abb)
\end{verbatim}
\item Check if the differences are statistically significant after adjusting for month length:
\begin{verbatim}
chisq.test(table(pisa$ST03Q01), p=monthdays/365)
\end{verbatim}
Why is it so?
...
barplot(rbind(male,female), names=month.abb, beside=TRUE, col=c("blue","red"))
abline(h=100/12)
\end{verbatim}
...
\item Write an R function \verb|statsplot| to compute the mean, min, max, and the quartiles, from a vector of data. The function should also produce a histogram of the data, with these statistics marked on the plot above the horizontal axis.
It should begin like this:
\begin{verbatim}
statsplot <- function(y)
{
  meany <- mean(y)
  
\end{verbatim}
You will need to search the R help facilities to find functions to compute each of the statistics. For the plot, the \verb|points| function may be useful.
